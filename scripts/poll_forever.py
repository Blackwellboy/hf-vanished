...
Keeps discovery cumulative and augments the core public-HF poller with the
metadata needed for recovery integrations, without adding credentials or a
backend service.
"""
...
def conservative_diff(prev: dict | None, cur: dict) -> list[tuple[str, str, str]]:
    changes = _original_diff(prev, cur)
...
def enrich_outputs() -> None:
    ...
    for event in envelope.get("events") or []:
        ...
        if event.get("curr") is not None and (event.get("curr") or {}).get("http") in (401, 403):
        ...
        if any(snap.get(k) is not None for k in ("sha", "license", "file_count", "weight_count", "weight_files")):
            event["last_public"] = {
                ...
                "library_name": snap.get("library_name"),
